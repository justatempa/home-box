...
function MoreInfo({ id }: { id: string }) {
  ...
  const [qrCodeData, setQRCodeData] = useState<{ qrCodeDataUrl: string; qrUrl: string } | null>(null);

  const generateQRCode = trpc.items.generateQRCode.useMutation({
    onSuccess: (data) => {
      setQRCodeData(data);
      setShowQRCode(true);
    },
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-white/60">物品二维码</div>
            <div className="mt-1 text-sm text-white/80">生成二维码以便快速分享物品信息</div>
          </div>
          <button
            onClick={() => generateQRCode.mutate({ itemId: id })}
            ...
            className={clsx(
              ...
              generateQRCode.isPending && "opacity-60",
            )}
          >
            {generateQRCode.isPending ? "生成中..." : "生成二维码"}
          </button>
        </div>
        ...
        {showQRCode && qrCodeData ? (
          <div className="mt-4 space-y-3">
            <div className="flex justify-center rounded-xl border border-white/10 bg-white p-4">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrCodeData.qrCodeDataUrl} alt="物品二维码" className="h-64 w-64" />
            </div>
            <div className="rounded-xl border border-white/10 bg-black/30 p-3">
              <div className="text-xs text-white/60">扫描链接</div>
              <div className="mt-1 break-all text-xs text-white/80">{qrCodeData.qrUrl}</div>
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/80 hover:bg-white/10"
              >
                下载二维码
              </button>
              <button
